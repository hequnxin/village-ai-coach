...
async function initGameData() {
  ...
  for (const t of themes) {
    await db.run(
      `INSERT INTO game_themes (id, name, icon, description, sort_order, is_active)
       VALUES ($1, $2, $3, $4, $5, 1)
      ...
      for (const lvl of levels) {
        ...
        if (candidates.length >= lvl.question_count) {
          ...
          for (const q of selected) {
            await db.run(
              `INSERT INTO game_level_questions (level_id, question_id)
               VALUES ($1, $2)
               ON CONFLICT (level_id, question_id) DO NOTHING`,
              [lvl.id, q.id]
            );
          }
        } else {
