...
// 勋章配置
const allBadges = [
  { name: '勤学好问', icon: '📚', condition: (stats) => stats.sessionCount >= 10, progress: (stats) => Math.min(100, stats.sessionCount / 10 * 100), current: (stats) => stats.sessionCount, target: 10, description: '对话次数达到10次' },
  { name: '知识贡献者', icon: '🏅', condition: (stats) => stats.approvedUploads >= 3, progress: (stats) => Math.min(100, stats.approvedUploads / 3 * 100), current: (stats) => stats.approvedUploads, target: 3, description: '上传的知识被采纳3次' },
  { name: '收藏达人', icon: '⭐', condition: (stats) => stats.favoriteCount >= 20, progress: (stats) => Math.min(100, stats.favoriteCount / 20 * 100), current: (stats) => stats.favoriteCount, target: 20, description: '收藏消息达到20条' },
  { name: '会话收藏家', icon: '💬', condition: (stats) => stats.favoriteSessionCount >= 5, progress: (stats) => Math.min(100, stats.favoriteSessionCount / 5 * 100), current: (stats) => stats.favoriteSessionCount, target: 5, description: '收藏会话达到5个' },
  { name: '对话达人', icon: '🗣️', condition: (stats) => stats.sessionCount >= 30, progress: (stats) => Math.min(100, stats.sessionCount / 30 * 100), current: (stats) => stats.sessionCount, target: 30, description: '对话次数达到30次' },
  { name: '挑战王者', icon: '👑', condition: (stats) => (stats.funCompleted || 0) >= 3, progress: (stats) => Math.min(100, ((stats.funCompleted || 0) / 3) * 100), current: (stats) => stats.funCompleted || 0, target: 3, description: '完成3个趣味闯关主题' }
];
...
// 修复后的成长曲线渲染函数（支持整数Y轴，并暴露到全局以便实时刷新）
      ...
      options: {
        ...
        scales: {
          y: {
            ...
            ticks: {
              callback: (value) => Math.floor(value) === value ? value : value.toFixed(0)
            }
          },
...
// 暴露刷新函数，供其他模块调用（例如积分添加后实时更新）
window.refreshGrowthChart = renderGrowthChart;
