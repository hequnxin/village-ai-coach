...
async function sendSimulateMessage(sessionId, text, container, roleName, targetVillager = null, isAuto = false) {
    ...
    if (!isAuto) {
        ...
        thinkingMsgDiv = createSimulateMessageElement('assistant', speakerName, speakerAvatar, '', 'neutral', undefined, null, true);
    ...
    try {
        ...
        if (!isAuto && thinkingMsgDiv) {
            const speakerName = targetVillager ? targetVillager.name : roleName;
            const speakerAvatar = targetVillager ? targetVillager.avatar : (roleName.includes('村民') ? '👵' : '🤖');
            const finalMsg = createSimulateMessageElement('assistant', speakerName, speakerAvatar, data.reply, data.emotion || 'neutral', data.satisfaction, data.messageId);
            ...
            const speakerName = targetVillager ? targetVillager.name : roleName;
            const speakerAvatar = targetVillager ? targetVillager.avatar : (roleName.includes('村民') ? '👵' : '🤖');
            const autoMsg = createSimulateMessageElement('assistant', speakerName, speakerAvatar, data.reply, data.emotion || 'neutral', data.satisfaction, data.messageId);
        ...
        if (!isAuto) {
            if (thinkingMsgDiv) {
                const errorMsg = createSimulateMessageElement('assistant', targetVillager ? targetVillager.name : roleName, targetVillager ? targetVillager.avatar : '🤖', `❌ 发送失败：${err.message}`, 'neutral');
...
export async function renderSimulateChat(session) {
                        ...
                        <div style="flex:1; display:flex; gap:8px; flex-wrap:wrap;">
                            ${stages.map((stage, idx) => `
                                <div style="display:flex; align-items:center; gap:4px;">
                                    ...
                                    <span style="font-size:0.75rem; ${stage.completed ? 'text-decoration:line-through; color:#888;' : (idx === currentStageIndex ? 'color:#ff9800; font-weight:bold;' : 'color:#333;')}">${stage.name}</span>
                                </div>
                            `).join('')}
                        </div>
                        <div style="font-size:0.7rem; color:#666;">进度：${completedCount}/${totalStages}</div>
                    </div>
                </div>
            `;
        }
    ...
    const sendMessage = async () => {
        ...
        await sendSimulateMessage(session.id, text, messagesContainer, scenario.role, target, false);
        ...
        tabs.forEach(tab => {
            tab.onclick = async () => {
                ...
                if (newTarget) {
                    ...
                    await sendSimulateMessage(session.id, `针对刚才提到的"${lastUserText}"，请${newTarget.name}说说你的看法。`, messagesContainer, scenario.role, newTarget, true);
                }
            };
        });
    }
    ...
    if (!hasSystemMsg && systemOpening) {
        ...
        if (!roundRobinDone) {
            roundRobinDone = true;
            await startRoundRobin(session.id, villagers, messagesContainer, scenario.role, loadingDiv);
        } else {
            loadingDiv.remove();
    ...
    // 从会话元数据中读取报告并添加“查看报告”按钮
    let savedReport = null;
    let savedFinalScore = null;
    let savedSatisfaction = null;
    let savedStagesCompleted = null;
    let savedTotalStages = null;
    if (session.scenarioId) {
        try {
            const extraData = JSON.parse(session.scenarioId);
            if (extraData.report) {
                savedReport = extraData.report;
                savedFinalScore = extraData.finalScore;
                savedSatisfaction = extraData.satisfaction;
                savedStagesCompleted = extraData.stagesCompleted;
                savedTotalStages = extraData.totalStages;
            }
        } catch(e) {}
    }

    if (savedReport && !document.getElementById('viewReportBtn')) {
        const toolbar = document.querySelector('.simulate-toolbar');
        if (toolbar) {
            const reportBtn = document.createElement('button');
            reportBtn.id = 'viewReportBtn';
            reportBtn.className = 'summary-btn';
            reportBtn.style.background = '#ff9800';
            reportBtn.style.color = 'white';
            reportBtn.innerHTML = '📋 查看本次对练报告';
            reportBtn.onclick = () => {
                showReportModal(savedReport, savedFinalScore * 20, savedSatisfaction, savedStagesCompleted, savedTotalStages);
            };
            const finishButton = document.getElementById('finishSimulateBtn');
            if (finishButton) {
                finishButton.parentNode.insertBefore(reportBtn, finishButton.nextSibling);
            } else {
