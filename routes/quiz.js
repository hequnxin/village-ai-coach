...
router.post('/daily/submit', async (req, res) => {
    ...
    for (let qid of wrongList) {
        ...
        if (existing) {
            ...
            await db.run('INSERT INTO wrong_questions (id, user_id, question_id, wrong_count, last_wrong_date) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), userId, qid, 1, today]);
        }
    }
    let rewardPoints = score * 10;
    if (score === answers.length) rewardPoints += 20;
    await db.run('INSERT INTO user_points (id, user_id, points, reason, created_at) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), userId, rewardPoints, `每日一练得分${score}/${answers.length}`, new Date().toISOString()]);
...
router.get('/fill-daily', async (req, res) => {
    ...
    if (!fillDaily) {
        ...
        const fillId = uuidv4();
        await db.run('INSERT INTO fill_daily (id, user_id, date, questions, completed, score) VALUES ($1, $2, $3, $4, $5, $6)', [fillId, userId, today, JSON.stringify(questions), 0, 0]);
...
router.post('/fill-daily/submit', async (req, res) => {
    ...
    if (rewardPoints > 0) {
        await db.run('INSERT INTO user_points (id, user_id, points, reason, created_at) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), userId, rewardPoints, `填空每日练答对${correctCount}题`, new Date().toISOString()]);
    }
...
router.post('/timed-challenge/submit', async (req, res) => {
    ...
    await db.run('INSERT INTO user_points (id, user_id, points, reason, created_at) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), userId, rewardPoints, `限时挑战答对${correctCount}题`, new Date().toISOString()]);
...
router.post('/shop/buy', async (req, res) => {
    ...
    await db.run('INSERT INTO user_points (id, user_id, points, reason, created_at) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), userId, -item.price, `购买商品：${item.name}`, new Date().toISOString()]);
    const userItemId = uuidv4();
    await db.run('INSERT INTO user_items (id, user_id, item_id, purchased_at, equipped) VALUES ($1, $2, $3, $4, $5)', [userItemId, userId, itemId, new Date().toISOString(), 0]);
...
router.post('/level/submit', async (req, res) => {
    ...
    if (existing) {
        ...
        await db.run('UPDATE user_level_progress SET completed = $1, score = $2, passed_at = $3 WHERE id = $4', [passed ? 1 : 0, correct, passed ? new Date().toISOString() : null, existing.id]);
    } else {
        await db.run('INSERT INTO user_level_progress (id, user_id, level_id, completed, score, passed_at) VALUES ($1, $2, $3, $4, $5, $6)', [uuidv4(), userId, levelId, passed ? 1 : 0, correct, passed ? new Date().toISOString() : null]);
    }
    if (passed) {
        await db.run('INSERT INTO user_points (id, user_id, points, reason, created_at) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), userId, 50, `通关关卡：${level.name}`, new Date().toISOString()]);
    }
...
router.get('/pk/status/:roomId', async (req, res) => {
    ...
    if (bothSubmitted && room.status === 'playing') {
        ...
        if (winner) {
            await db.run('INSERT INTO user_points (id, user_id, points, reason, created_at) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), winner, 30, 'PK胜利', new Date().toISOString()]);
        }
    }
    res.json({
        status: room.status,
...
router.post('/fill-submit', async (req, res) => {
    ...
    if (isCorrect) {
        await db.run('INSERT INTO user_points (id, user_id, points, reason, created_at) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), req.user.userId, 10, '填空游戏答对', new Date().toISOString()]);
    }
...
router.post('/wrong-questions/clear', async (req, res) => {
    ...
    if (allCorrect) {
        ...
        await db.run('INSERT INTO user_points (id, user_id, points, reason, created_at) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), userId, 50, '错题清零', new Date().toISOString()]);
...
router.get('/weekly/current', async (req, res) => {
    ...
    if (!contest) {
        ...
        await db.run('INSERT INTO weekly_contest (id, week_start, week_end, questions, status) VALUES ($1, $2, $3, $4, $5)', [contestId, startStr, endStr, JSON.stringify(questionIds), 'active']);
...
router.post('/weekly/submit', async (req, res) => {
    ...
    if (existing) {
        await db.run('UPDATE weekly_contest_scores SET score = $1, time_used = $2, submitted_at = $3 WHERE id = $4', [correct, timeUsed, new Date().toISOString(), existing.id]);
    } else {
        await db.run('INSERT INTO weekly_contest_scores (id, contest_id, user_id, score, time_used, submitted_at) VALUES ($1, $2, $3, $4, $5, $6)', [uuidv4(), contestId, userId, correct, timeUsed, new Date().toISOString()]);
    }
    const points = correct * 5;
    await db.run('INSERT INTO user_points (id, user_id, points, reason, created_at) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), userId, points, `每周竞赛得分${correct}`, new Date().toISOString()]);
...
router.get('/scratch/generate', async (req, res) => {
    ...
    const cardId = uuidv4();
    await db.run('INSERT INTO scratch_cards (id, user_id, question_id, answer, reward_points, created_at) VALUES ($1, $2, $3, $4, $5, $6)', [cardId, userId, question.id, question.answer, 0, new Date().toISOString()]);
...
router.post('/add-points', async (req, res) => {
    ...
    await db.run('INSERT INTO user_points (id, user_id, points, reason, created_at) VALUES ($1, $2, $3, $4, $5)', [uuidv4(), userId, points, reason || '游戏奖励', new Date().toISOString()]);
    res.json({ success: true });
});
