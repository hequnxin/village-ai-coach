...
router.get('/pk/status/:roomId', async (req, res) => {
  ...
  if (bothSubmitted && room.status === 'playing') {
    ...
    await db.run('UPDATE pk_rooms SET status = $1, winner_id = $2, finished_at = $3 WHERE id = $4',
      ['finished', winner, new Date().toISOString(), room.id]);
    if (winner) {
      await db.run('INSERT INTO user_points (id, user_id, points, reason, created_at) VALUES ($1, $2, $3, $4, $5)',
...
router.post('/questions/batch', async (req, res) => {
  ...
  const sql = `SELECT id, type, question, options, answer, explanation, category FROM quiz_questions WHERE id IN (${placeholders})`;
  const questions = await db.all(sql, ids);
