...
{"scores": {"沟通技巧": 4, ...}, "suggestions": "整体建议...", "examples": [{"quote": "用户原话", "verdict": "优点/需改进", "comment": "具体点评"}], "bestPractices": ["优秀话术1", "优秀话术2"], "mistakes": ["失误点1", "失误点2"]}`;
    ...
    try {
        ...
        // ★ 修改：不再保存报告为系统消息，改为存入会话的 scenario_id 中
        ...
        const finalScore5 = Math.min(5, totalScore + satisfactionBonus/10);
        // 转换为百分制
        const finalScore100 = Math.round(finalScore5 * 20);

        // 将报告数据合并到 extra 中
        extra.report = report;
        extra.finalScore = finalScore5;
        extra.finalScore100 = finalScore100;
        extra.satisfaction = finalSatisfaction;
        extra.stagesCompleted = stages.filter(s => s.completed).length;
        extra.totalStages = stages.length;
        await db.run(`UPDATE sessions SET scenario_id = $1 WHERE id = $2`, [JSON.stringify(extra), sessionId]);
